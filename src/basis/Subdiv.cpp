...
void MeshWithConnectivity::LoopSubdivision() {
	...
	for (int i = 0; i < (int)indices.size(); ++i) {
		for (int j = 0; j < 3; ++j) {
			...
			do {
				...
				Vec3i triangle = indices[near_tri];
				near_vertex.push_back(triangle[(near_edge + 2) % 3]);
			...
			if (boundary) {
				...
				for (int k = 0; k < near_vertex.size(); k++) {
					pos += B * positions[near_vertex[k]];
					col += B * colors[near_vertex[k]];
	...
	for (size_t i = 0; i < indices.size(); ++i) {
		...
		auto edge_b = std::make_pair(min(even[2], even[1]), max(even[2], even[1]));
		auto edge_c = std::make_pair(min(even[0], even[2]), max(even[0], even[2]));
